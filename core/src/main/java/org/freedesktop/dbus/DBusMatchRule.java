...
public class DBusMatchRule
{
	...
	public DBusMatchRule(Class<? extends Object> c, String source,
			String object) throws DBusException
	{
		this(c);
		this.source = source;
		this.object = object;
	...
	public DBusMatchRule(Class<? extends Object> c) throws DBusException
	{
		if (DBusInterface.class.isAssignableFrom(c)) {
			if (null != c.getAnnotation(DBusInterfaceName.class))
				...
				iface = AbstractConnection.dollar_pattern.matcher(c.getName())
						.replaceAll(".");
			if (!iface.matches(".*\\..*"))
				throw new DBusException(
						_("DBusInterfaces must be defined in a package."));
			member = null;
			type = null;
			...
			else if (null != c.getEnclosingClass()
					.getAnnotation(DBusInterfaceName.class))
				iface = c.getEnclosingClass()
						.getAnnotation(DBusInterfaceName.class).value();
			else
				iface = AbstractConnection.dollar_pattern
			...
			signalTypeMap.put(iface + '$' + member,
			...
			if (null != c.getClass().getAnnotation(DBusInterfaceName.class))
				iface = c.getClass().getAnnotation(DBusInterfaceName.class)
						.value();
			else
				iface = AbstractConnection.dollar_pattern
						.matcher(c.getClass().getName()).replaceAll(".");
			if (!iface.matches(".*\\..*"))
				throw new DBusException(
						_("DBusInterfaces must be defined in a package."));
			member = null;
	...
	public String toString()
	{
		...
		if (null != member)
			s = null == s ? "member='" + member + "'"
					: s + ",member='" + member + "'";
		if (null != iface)
			s = null == s ? "interface='" + iface + "'"
					: s + ",interface='" + iface + "'";
		if (null != source)
			s = null == s ? "sender='" + source + "'"
					: s + ",sender='" + source + "'";
		if (null != object)
			s = null == s ? "path='" + object + "'"
					: s + ",path='" + object + "'";
		return s;
	}
