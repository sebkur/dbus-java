...
class ExportedObject
{
	...
	private String getAnnotations(AnnotatedElement c)
	{
		...
		for (Annotation a : c.getDeclaredAnnotations()) {
			...
			ans += "  <annotation name=\""
					+ AbstractConnection.dollar_pattern.matcher(t.getName())
							.replaceAll(".")
					+ "\" value=\"" + value + "\" />\n";
		}
		return ans;
	...
	@SuppressWarnings("unchecked")
	private Map<MethodTuple, Method> getExportedMethods(Class c)
		...
		for (Class i : c.getInterfaces())
			if (DBusInterface.class.equals(i)) {
				...
				if (null != c.getAnnotation(DBusInterfaceName.class)) {
					String name = ((DBusInterfaceName) c
					...
					if (c.getName().equals(c.getSimpleName()))
						throw new DBusException(_(
								"DBusInterfaces cannot be declared outside a package"));
					if (c.getName().length() > DBusConnection.MAX_NAME_LENGTH)
						throw new DBusException(
								_("Introspected interface name exceeds 255 characters. Cannot export objects of type ")
										+ c.getName());
					else
						introspectiondata += " <interface name=\""
								+ AbstractConnection.dollar_pattern
										.matcher(c.getName()).replaceAll(".")
								+ "\">\n";
				}
				...
				for (Method meth : c.getDeclaredMethods())
					if (Modifier.isPublic(meth.getModifiers())) {
						...
						if (meth.isAnnotationPresent(DBusMemberName.class))
							name = meth.getAnnotation(DBusMemberName.class)
									.value();
						else
							name = meth.getName();
						if (name.length() > DBusConnection.MAX_NAME_LENGTH)
							throw new DBusException(
									_("Introspected method name exceeds 255 characters. Cannot export objects with method ")
											+ name);
						introspectiondata += "  <method name=\"" + name
								+ "\" >\n";
						introspectiondata += getAnnotations(meth);
						for (Class ex : meth.getExceptionTypes())
							if (DBusExecutionException.class
									.isAssignableFrom(ex))
						...
						for (Type pt : meth.getGenericParameterTypes())
							for (String s : Marshalling.getDBusType(pt)) {
								introspectiondata += "   <arg type=\"" + s
										+ "\" direction=\"in\"/>\n";
								ms += s;
							}
						if (!Void.TYPE.equals(meth.getGenericReturnType())) {
							if (Tuple.class.isAssignableFrom(
									(Class) meth.getReturnType())) {
								ParameterizedType tc = (ParameterizedType) meth
								...
								for (Type t : ts)
									if (t != null)
										for (String s : Marshalling
												.getDBusType(t))
											introspectiondata += "   <arg type=\""
													+ s
													+ "\" direction=\"out\"/>\n";
							} else if (Object[].class
									.equals(meth.getGenericReturnType())) {
								throw new DBusException(_(
										"Return type of Object[] cannot be introspected properly"));
							} else
								for (String s : Marshalling.getDBusType(
				...
				for (Class sig : c.getDeclaredClasses())
					if (DBusSignal.class.isAssignableFrom(sig)) {
						...
						if (sig.isAnnotationPresent(DBusMemberName.class)) {
							name = ((DBusMemberName) sig
									.getAnnotation(DBusMemberName.class))
											.value();
						...
						if (name.length() > DBusConnection.MAX_NAME_LENGTH)
							throw new DBusException(
									_("Introspected signal name exceeds 255 characters. Cannot export objects with signals of type ")
											+ name);
	...
	public ExportedObject(DBusInterface object, boolean weakreferences)
			throws DBusException
	{
		if (weakreferences)
