...
class RemoteInvocationHandler implements InvocationHandler
{
	...
	final static Logger logger = LoggerFactory
			.getLogger(RemoteInvocationHandler.class);
	...
	public static Object convertRV(String sig, Object[] rp, Method m,
			AbstractConnection conn) throws DBusException
	{
		...
		if (null == rp) {
			...
			try {
				logger.trace("Converting return parameters from "
						+ Arrays.deepToString(rp) + " to type "
		...
		default:
			...
			if (!Tuple.class.isAssignableFrom(c)) {
				throw new DBusExecutionException(
						_("Wrong return type (not expecting Tuple)"));
			}
	...
	public static Object executeRemoteMethod(RemoteObject ro, Method m,
			AbstractConnection conn, int syncmethod, CallbackHandler callback,
			Object... args) throws DBusExecutionException
	{
		...
		if (ts.length > 0) {
			try {
				...
				throw new DBusExecutionException(
						_("Failed to construct D-Bus type: ")
								+ DBe.getMessage());
			}
		}
		MethodCall call;
		...
		try {
			...
			throw new DBusExecutionException(
					_("Failed to construct outgoing method call: ")
							+ DBe.getMessage());
		}
		if (null == conn.outgoing) {
			throw new NotConnected(_("Not Connected"));
		}
		...
		case CALL_TYPE_CALLBACK:
			synchronized (conn.pendingCallbacks) {
				...
				conn.pendingCallbackReplys.put(call,
						new DBusAsyncReply(call, m, conn));
			}
			conn.queueOutgoing(call);
			return null;
		case CALL_TYPE_SYNC:
		...
		if (null == reply) {
			throw new DBus.Error.NoReply(_("No reply within specified time"));
		}
			...
			throws Throwable
	{
		if (method.getName().equals("isRemote")) {
			...
			try {
				if (1 == args.length) {
					return new Boolean(
							remote.equals(((RemoteInvocationHandler) Proxy
									.getInvocationHandler(args[0])).remote));
				}
			...
			} else if (2 == args.length && args[0] instanceof Long
		...
		return executeRemoteMethod(remote, method, conn, CALL_TYPE_SYNC, null,
				args);
	}
}
