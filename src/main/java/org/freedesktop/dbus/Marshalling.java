...
public class Marshalling
{
			...
			int level) throws DBusException
	{
		...
		if (Debug.debug)
			Debug.print(Debug.VERBOSE, "Converted Java type: " + c
					+ " to D-Bus Type: " + out[level]);
			...
			AbstractConnection conn) throws DBusException
	{
		...
		for (int i = 0; i < parameters.length; i++) {
			if (Debug.debug)
				Debug.print(Debug.VERBOSE, "Converting " + i + " from "
						+ parameters[i] + " to " + types[i]);
			...
			if (parameters[i] instanceof DBusSerializable) {
				...
				if (Debug.debug)
					Debug.print(Debug.VERBOSE,
							"New params: " + Arrays.deepToString(parameters)
									+ " new types: "
	...
	{
		if (Debug.debug)
			Debug.print(Debug.VERBOSE, "Deserializing from "
					+ parameter.getClass() + " to " + type.getClass());
		if (null == parameter)
			return null;
		...
		if (parameter instanceof Object[] && type instanceof Class
			...
			if (Debug.debug)
				Debug.print(Debug.VERBOSE,
						"Creating Struct " + type + " from " + parameter);
			...
			Type type, AbstractConnection conn) throws Exception
	{
		if (Debug.debug)
			Debug.print(Debug.VERBOSE,
					"Deserializing from " + parameters + " to " + type);
		if (null == parameters)
			return null;
			...
			AbstractConnection conn) throws Exception
	{
		if (Debug.debug)
			Debug.print(Debug.VERBOSE,
					"Deserializing from " + Arrays.deepToString(parameters)
				...
				if (Debug.debug) {
					for (int j = 0; j < parameters.length; j++) {
						Debug.print(Debug.ERR,
								String.format(
										"Error, Parameters difference (%1d, '%2s')",
										j, parameters[j].toString()));
					}
				}
				throw new DBusException(_(
