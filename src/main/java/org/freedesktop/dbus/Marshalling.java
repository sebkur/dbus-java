...
public class Marshalling
{
	...
	final static Logger logger = LoggerFactory.getLogger(Marshalling.class);

	private static Map<Type, String[]> typeCache = new HashMap<Type, String[]>();

	/**
	 * Will return the DBus type corresponding to the given Java type. Note,
	 * container type should have their ParameterizedType not their Class passed
	 * in here.
	 * 
	 * @param c
	 *            The Java types.
	 ...
	 *            The Java type.
	 * @param basic
	 *            If true enforces this to be a non-compound type. (compound
	 *            types are Maps, Structs and Lists/arrays).
	 * @return The DBus type.
	 * @throws DBusException
	 *             If the given type cannot be converted to a DBus type.
	 */
	...
	public static String[] recursiveGetDBusType(Type c, boolean basic,
			int level) throws DBusException
	{
		if (out.length <= level) {
		...
		if (c instanceof TypeVariable) {
			...
			String[] s = recursiveGetDBusType(
					((GenericArrayType) c).getGenericComponentType(), false,
					level + 1);
			if (s.length != 1) {
				throw new DBusException(
						_("Multi-valued array types not permitted"));
			}
			out[level].append(s[0]);
		} else if ((c instanceof Class && DBusSerializable.class
				.isAssignableFrom((Class<? extends Object>) c))
				|| (c instanceof ParameterizedType
						&& DBusSerializable.class.isAssignableFrom(
								(Class<? extends Object>) ((ParameterizedType) c)
										.getRawType()))) {
			// it's a custom serializable type
			Type[] newtypes = null;
			if (c instanceof Class) {
				for (Method m : ((Class<? extends Object>) c)
						.getDeclaredMethods()) {
				...
				for (Method m : ((Class<? extends Object>) ((ParameterizedType) c)
						.getRawType()).getDeclaredMethods()) {
					if (m.getName().equals("deserialize")) {
						newtypes = m.getGenericParameterTypes();
					}
				}
			...
			for (int j = 0; j < sigs.length; j++) {
				...
				if (1 != ss.length) {
					throw new DBusException(_(
							"Serializable classes must serialize to native DBus types"));
				}
				sigs[j] = ss[0];
			}
			return sigs;
			...
			if (p.getRawType().equals(Map.class)) {
				...
				for (Type t : p.getActualTypeArguments()) {
					if (Type.class.equals(t)) {
						out[level]
								.append((char) Message.ArgumentType.SIGNATURE);
					} else {
						String[] s = recursiveGetDBusType(t, false, level + 1);
						if (s.length != 1) {
							throw new DBusException(_(
									"Multi-valued array types not permitted"));
						}
						out[level].append((char) Message.ArgumentType.ARRAY);
						out[level].append(s[0]);
					}
				...
				out[level].append((char) Message.ArgumentType.VARIANT);
			} else if (DBusInterface.class.isAssignableFrom(
					(Class<? extends Object>) p.getRawType())) {
				...
				throw new DBusException(
						_("Exporting non-exportable parameterized type ") + c);
			}
		}
			...
			out[level].append((char) Message.ArgumentType.VARIANT);
		} else if (c instanceof Class && DBusInterface.class
				.isAssignableFrom((Class<? extends Object>) c)) {
			out[level].append((char) Message.ArgumentType.OBJECT_PATH);
			...
			if (Type.class
					.equals(((Class<? extends Object>) c).getComponentType())) {
				out[level].append((char) Message.ArgumentType.SIGNATURE);
			} else {
				out[level].append((char) Message.ArgumentType.ARRAY);
				String[] s = recursiveGetDBusType(
						((Class<? extends Object>) c).getComponentType(), false,
						level + 1);
				if (s.length != 1) {
					throw new DBusException(
							_("Multi-valued array types not permitted"));
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Converted Java type: " + c + " to D-Bus Type: "
					+ out[level]);
		}
			...
			for (; i < dbus.length()
				...
				case Message.ArgumentType.STRUCT1:
					...
					for (int c = 1; c > 0; j++) {
						if (')' == dbus.charAt(j)) {
							c--;
						} else if (Message.ArgumentType.STRUCT1 == dbus
								.charAt(j)) {
							c++;
						}
					}
				...
				default:
					throw new DBusException(MessageFormat.format(
							_("Failed to parse DBus type signature: {0} ({1})."),
							new Object[] { dbus, dbus.charAt(i) }));
				}
			}
			return i;
		} catch (IndexOutOfBoundsException IOOBe) {
			if (AbstractConnection.EXCEPTION_DEBUG && logger.isErrorEnabled()) {
				logger.error("", IOOBe);
			}
			throw new DBusException(
					_("Failed to parse DBus type signature: ") + dbus);
		}
	}
			...
			AbstractConnection conn) throws DBusException
	{
		...
		for (int i = 0; i < parameters.length; i++) {
			if (logger.isTraceEnabled()) {
				logger.trace("Converting " + i + " from " + parameters[i]
						+ " to " + types[i]);
			}
			...
			if (parameters[i] instanceof DBusSerializable) {
				for (Method m : parameters[i].getClass().getDeclaredMethods()) {
					if (m.getName().equals("deserialize")) {
						...
						Type[] expand = new Type[types.length + newtypes.length
								- 1];
						System.arraycopy(types, 0, expand, 0, i);
						System.arraycopy(newtypes, 0, expand, i,
								newtypes.length);
						System.arraycopy(types, i + 1, expand,
								i + newtypes.length, types.length - i - 1);
						types = expand;
						Object[] newparams = ((DBusSerializable) parameters[i])
								.serialize();
						Object[] exparams = new Object[parameters.length
								+ newparams.length - 1];
						System.arraycopy(parameters, 0, exparams, 0, i);
						System.arraycopy(newparams, 0, exparams, i,
								newparams.length);
						System.arraycopy(parameters, i + 1, exparams,
								i + newparams.length,
				...
				System.arraycopy(parameters, i + 1, exparams,
						i + newparams.length, parameters.length - i - 1);
				parameters = exparams;
				if (logger.isTraceEnabled()) {
					logger.trace("New params: "
							+ Arrays.deepToString(parameters) + " new types: "
				...
				parameters[i] = conn
						.getExportedObject((DBusInterface) parameters[i]);
			}
		}
		return parameters;
	}
	...
	@SuppressWarnings("unchecked")
	static Object deSerializeParameter(Object parameter, Type type,
			AbstractConnection conn) throws Exception
	{
		if (logger.isTraceEnabled()) {
			logger.trace("Deserializing from " + parameter.getClass() + " to "
					+ type.getClass());
		}
		if (null == parameter) {
			return null;
		...
		if (parameter instanceof ObjectPath) {
			if (type instanceof Class
					&& DBusInterface.class.isAssignableFrom((Class) type)) {
		...
		if (parameter instanceof Object[] && type instanceof Class
		...
		if (parameter instanceof List) {
			...
			if (null != type2) {
				parameter = deSerializeParameters((List) parameter, type2,
						conn);
			}
		}
		...
		if (parameter instanceof Object[] || parameter instanceof List
				|| parameter.getClass().isArray()) {
			if (type instanceof ParameterizedType) {
				parameter = ArrayFrob.convert(parameter,
						(Class<? extends Object>) ((ParameterizedType) type)
								.getRawType());
			} else if (type instanceof GenericArrayType) {
		...
		if (parameter instanceof DBusMap) {
			...
			Type[] maptypes = ((ParameterizedType) type)
					.getActualTypeArguments();
			for (int i = 0; i < dmap.entries.length; i++) {
				dmap.entries[i][0] = deSerializeParameter(dmap.entries[i][0],
						maptypes[0], conn);
				dmap.entries[i][1] = deSerializeParameter(dmap.entries[i][1],
						maptypes[1], conn);
			}
		}
		return parameter;
	...
	static List<Object> deSerializeParameters(List<Object> parameters,
			Type type, AbstractConnection conn) throws Exception
	{
		if (logger.isTraceEnabled()) {
		...
		for (int i = 0; i < parameters.size(); i++) {
			...
			/*
			 * DO NOT DO THIS! IT'S REALLY NOT SUPPORTED! if (type instanceof
			 * Class && DBusSerializable.class.isAssignableFrom((Class)
			 * types[i])) { for (Method m: ((Class)
			 * types[i]).getDeclaredMethods()) if
			 * (m.getName().equals("deserialize")) { Type[] newtypes =
			 * m.getGenericParameterTypes(); try { Object[] sub = new
			 * Object[newtypes.length]; System.arraycopy(parameters, i, sub, 0,
			 * newtypes.length); sub = deSerializeParameters(sub, newtypes,
			 * conn); DBusSerializable sz = (DBusSerializable) ((Class)
			 * types[i]).newInstance(); m.invoke(sz, sub); Object[] compress =
			 * new Object[parameters.length - newtypes.length + 1];
			 * System.arraycopy(parameters, 0, compress, 0, i); compress[i] =
			 * sz; System.arraycopy(parameters, i + newtypes.length, compress,
			 * i+1, parameters.length - i - newtypes.length); parameters =
			 * compress; } catch (ArrayIndexOutOfBoundsException AIOOBe) { if
			 * (AbstractConnection.EXCEPTION_DEBUG && Debug.debug)
			 * Debug.print(Debug.ERR, AIOOBe); throw new
			 * DBusException("Not enough elements to create custom object from serialized data ("
			 * +(parameters.size()-i)+" < "+(newtypes.length)+")"); } } } else
			 */
			parameters.set(i,
					deSerializeParameter(parameters.get(i), type, conn));
		}
		return parameters;
	}

	@SuppressWarnings("unchecked")
	static Object[] deSerializeParameters(Object[] parameters, Type[] types,
			AbstractConnection conn) throws Exception
	{
		if (logger.isTraceEnabled()) {
			logger.trace("Deserializing from " + Arrays.deepToString(parameters)
					+ " to " + Arrays.deepToString(types));
		}
		if (null == parameters) {
			return null;
		}
		...
		for (int i = 0; i < parameters.length; i++) {
			// CHECK IF ARRAYS HAVE THE SAME LENGTH <-- has to happen after
			// expanding parameters
			if (i >= types.length) {
				if (logger.isErrorEnabled()) {
					for (int j = 0; j < parameters.length; j++) {
						logger.error(String.format(
								"Error, Parameters difference (%1d, '%2s')", j,
								parameters[j].toString()));
					}
				}
				throw new DBusException(_(
						"Error deserializing message: number of parameters didn't match receiving signature"));
			}
			if (null == parameters[i]) {
				continue;
			}

			if ((types[i] instanceof Class && DBusSerializable.class
					.isAssignableFrom((Class<? extends Object>) types[i]))
					|| (types[i] instanceof ParameterizedType
							&& DBusSerializable.class.isAssignableFrom(
									(Class<? extends Object>) ((ParameterizedType) types[i])
											.getRawType()))) {
				Class<? extends DBusSerializable> dsc;
				if (types[i] instanceof Class) {
					dsc = (Class<? extends DBusSerializable>) types[i];
				} else {
					dsc = (Class<? extends DBusSerializable>) ((ParameterizedType) types[i])
							.getRawType();
				}
				for (Method m : dsc.getDeclaredMethods()) {
					if (m.getName().equals("deserialize")) {
						...
						try {
							...
							System.arraycopy(parameters, i, sub, 0,
									newtypes.length);
							sub = deSerializeParameters(sub, newtypes, conn);
							DBusSerializable sz = dsc.newInstance();
							m.invoke(sz, sub);
							Object[] compress = new Object[parameters.length
									- newtypes.length + 1];
							System.arraycopy(parameters, 0, compress, 0, i);
							compress[i] = sz;
							System.arraycopy(parameters, i + newtypes.length,
									compress, i + 1,
							...
							throw new DBusException(MessageFormat.format(
									...
									new Object[] { parameters.length - i,
											newtypes.length }));
						}
					}
				}
