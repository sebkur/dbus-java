...
abstract class TabbedSaveAction extends AbstractAction
		implements Iterable<TextFile>
{
	...
	protected TextFile getTextFile(int index)
	{
		...
		JTextComponent textComponent = (JTextComponent) scrollPane.getViewport()
				.getView();
	...
	public final void actionPerformed(ActionEvent e)
	{
		...
		if (chooser == null) {
			/**
			 * Occurs on event dispatch thread, so no problems with lazy static
			 * init here
			 */
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			if (parentDirectory.exists() || parentDirectory.mkdirs()) {
				if (parentDirectory.canWrite()) {
					Runnable runnable = new FileSaver(tabbedPane,
							parentDirectory, this);

					new Thread(runnable).start();
				} else {
					JOptionPane.showMessageDialog(tabbedPane,
							_("Could not write to parent directory"),
							_("Invalid Parent Directory"),
							JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(tabbedPane,
						_("Could not access parent directory"),
						_("Invalid Parent Directory"),
						JOptionPane.ERROR_MESSAGE);
			}
		}
