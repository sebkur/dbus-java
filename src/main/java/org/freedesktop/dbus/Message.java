...
public class Message
{
	...
	/**
	 * Defines constants for each argument type. There are two constants for
	 * each argument type, as a byte or as a String (the _STRING version)
	 */
	...
	static {
		padding = new byte[][] { null, new byte[1], new byte[2], new byte[3],
	...
	void populate(byte[] msg, byte[] headers, byte[] body) throws DBusException
	{
		...
		this.body = body;
		bufferuse = 3;
		bodylen = ((Number) extract(Message.ArgumentType.UINT32_STRING, msg,
				4)[0]).longValue();
	...
	{
		return big ? demarshallintBig(buf, ofs, width)
				: demarshallintLittle(buf, ofs, width);
	}
	 ...
	 *            The byte-width of the int.
	 */
	public static long demarshallint(byte[] buf, int ofs, byte endian,
			int width)
	{
		return endian == Endian.BIG ? demarshallintBig(buf, ofs, width)
				: demarshallintLittle(buf, ofs, width);
	}
	...
	/**
	 * Appends a value to the message. The type of the value is read from a
	 * D-Bus signature and used to marshall the value.
	 * 
	 * @param sigb
		...
		try {
				...
				if (data.getClass().isArray()
					...
					default:
						throw new MarshallingException(_(
								"Primative array being sent as non-primative array."));
					}
					appendBytes(primbuf);
			...
			case ArgumentType.VARIANT:
				...
				if (data instanceof Variant) {
					...
					appendone(new byte[] { ArgumentType.SIGNATURE }, 0,
							var.getSig());
					...
					appendone(new byte[] { ArgumentType.SIGNATURE }, 0,
							contents[0]);
					appendone(((String) contents[0]).getBytes(), 0,
							contents[1]);
				} else {
	 ...
	 *            The buffer to demarshall from.
	 * @param ofs
	 *            An array of two ints, the offset into the signature buffer and
	 *            the offset into the data buffer. These values will be updated
	 *            to the start of the next value ofter demarshalling.
	 * @param contained
			...
			boolean contained) throws DBusException
	{
		...
		case ArgumentType.ARRAY:
			...
			case ArgumentType.DICT_ENTRY1:
				if (0 == size) {
					...
					String temp3 = new String(temp2);
					// ofs[0] gets incremented anyway. Leave one character on
					// the stack
				...
				rv = new DBusMap<Object, Object>(
						entries.toArray(new Object[0][]));
				break;
			default:
				if (0 == size) {
					// advance the type parser even on 0-size arrays.
		...
		case ArgumentType.VARIANT:
			...
			String sig = (String) extract(ArgumentType.SIGNATURE_STRING, buf,
					newofs)[0];
			newofs[0] = 0;
	 ...
	 *            The buffer to demarshall from.
	 * @param ofs
	 *            An array of two ints, the offset into the signature and the
	 *            offset into the data buffer. These values will be updated to
	 *            the start of the next value ofter demarshalling.
	 * @return The demarshalled value(s).
	 */
	public Object[] extract(String sig, byte[] buf, int[] ofs)
			throws DBusException
	...
	/**
	 * Warning, do not use this method unless you really know what you are
	 * doing.
	 */
	public void setSource(String source) throws DBusException
	{
		if (null != body) {
			...
			bytecounter = 0;
			preallocate(12);
			append("yyyyuu", big ? Endian.BIG : Endian.LITTLE, type, flags,
					protover, bodylen, serial);
