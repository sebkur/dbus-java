...
public abstract class AbstractConnection
{
	protected class FallbackContainer
	{
		...
		{
			if (Debug.debug)
				Debug.print(Debug.DEBUG, "Removing fallback on " + path);
		...
		public synchronized ExportedObject get(String path)
		{
			...
			for (String[] fbpath : fallbacks.keySet()) {
				if (Debug.debug)
					Debug.print(Debug.VERBOSE,
							"Trying fallback path "
									+ Arrays.deepToString(fbpath) + " to match "
				...
				if (Debug.debug)
					Debug.print(Debug.VERBOSE,
							"Matches " + i + " bestobject now " + bestobject);
			}
			if (Debug.debug)
				Debug.print(Debug.DEBUG,
						"Found fallback for " + path + " of " + bestobject);
			return bestobject;
		}
	}
		...
		public void run()
		{
			try {
				...
				while (_run) {
					...
					// read from the wire
					try {
						// this blocks on outgoing being non-empty or a message
						// being available.
						m = readIncoming();
						if (m != null) {
							if (Debug.debug)
								Debug.print(Debug.VERBOSE,
										"Got Incoming Message: " + m);
							synchronized (this) {
								notifyAll();
							}
				...
				if (Debug.debug && EXCEPTION_DEBUG)
					Debug.print(Debug.ERR, e);
			}
		}
	}

	private class _globalhandler implements org.freedesktop.DBus.Peer,
	...
	private class _sender extends Thread
	{
		...
		public void run()
		{
			...
			if (Debug.debug)
				Debug.print(Debug.INFO, "Monitoring outbound queue");
			// block on the outbound queue and send from it
			while (_run) {
				if (null != outgoing)
					synchronized (outgoing) {
						if (Debug.debug)
							Debug.print(Debug.VERBOSE, "Blocking");
						...
						if (Debug.debug)
							Debug.print(Debug.VERBOSE, "Notified");
						if (outgoing.size() > 0)
							m = outgoing.remove();
						if (Debug.debug)
							Debug.print(Debug.DEBUG, "Got message: " + m);
					}
				if (null != m)
					sendMessage(m);
				m = null;
			...
			if (Debug.debug)
				Debug.print(Debug.INFO, "Flushing outbound queue and quitting");
	...
	static {
		...
		if (Debug.debug) {
			...
			if (f.exists()) {
				...
				Debug.print("debug config file " + f
						+ " does not exist, not loading.");
			}
	...
	String getExportedObject(DBusInterface i) throws DBusException
	{
		...
		throw new DBusException(
				"Not an object exported or imported by this connection");
	}

	abstract DBusInterface getExportedObject(String source, String path)
			throws DBusException;

	/**
	 * Returns a structure with information on the current method call.
	 * 
	 * @return the DBusCallInfo for this method call, or null if we are not in a
	 *         method call.
	 */
	...
	/**
	 * If set to true the bus will not hold a strong reference to exported
	 * objects. If they go out of scope they will automatically be unexported
	 * from the bus. The default is to hold a strong reference, which means
	 * objects must be explicitly unexported before they will be garbage
	 * collected.
	 */
	 ...
	 * 
	 * @param objectpath
	 *            The path to the object we are exposing. MUST be in
	 *            slash-notation, like "/org/freedesktop/Local", and SHOULD end
	 *            with a capitalised term. Only one object may be exposed on
	 *            each path at any one time, but an object may be exposed on
	 *            several paths at once.
	 * @param object
	 *            The object to export.
	 * @throws DBusException
	 *             If the objectpath is already exporting an object. or if
	 *             objectpath is incorrectly formatted,
	 */
	public void exportObject(String objectpath, DBusInterface object)
			throws DBusException
	{
	...
	/**
	 * Export an object as a fallback object. This object will have it's methods
	 * invoked for all paths starting with this object path.
	 * 
	 * @param objectprefix
	 *            The path below which the fallback handles calls. MUST be in
	 *            slash-notation, like "/org/freedesktop/Local",
	 * @param object
	 *            The object to export.
	 * @throws DBusException
	 *             If the objectpath is incorrectly formatted,
	 */
	public void addFallback(String objectprefix, DBusInterface object)
			throws DBusException
	{
		if (null == objectprefix || "".equals(objectprefix))
			throw new DBusException(_("Must Specify an Object Path"));
		if (!objectprefix.matches(OBJECT_REGEX)
				|| objectprefix.length() > MAX_NAME_LENGTH)
	...
	/**
	 * Return a reference to a remote object. This method will resolve the well
	 * known name (if given) to a unique bus name when you call it. This means
	 * that if a well known name is released by one process and acquired by
	 * another calls to objects gained from this method will continue to operate
	 * on the original process.
	 * 
	 * @param busname
	 *            The bus name to connect to. Usually a well known bus name in
	 *            dot-notation (such as "org.freedesktop.local") or may be a
	 *            DBus address such as ":1-16".
	 * @param objectpath
	 *            The path on which the process is exporting the object.$
	 * @param type
	 *            The interface they are exporting it on. This type must have
	 *            the same full class name and exposed method signatures as the
	 *            interface the remote object is exporting.
	 * @return A reference to a remote object.
	 * @throws ClassCastException
	 *             If type is not a sub-type of DBusInterface
	 * @throws DBusException
	 *             If busname or objectpath are incorrectly formatted or type is
	 *             not in a package.
	 */
	/**
	 * Send a signal.
	...
	void queueOutgoing(Message m)
	{
		synchronized (outgoing) {
			...
			outgoing.add(m);
			if (Debug.debug)
				Debug.print(Debug.DEBUG, "Notifying outgoing thread");
			outgoing.notifyAll();
		}
	}
	 ...
	 */
	public <T extends DBusSignal> void removeSigHandler(Class<T> type,
			DBusInterface object, DBusSigHandler<T> handler)
			throws DBusException
	{
		if (!DBusSignal.class.isAssignableFrom(type))
			throw new ClassCastException(_("Not A DBus Signal"));
	...
	protected abstract <T extends DBusSignal> void removeSigHandler(
			DBusMatchRule rule, DBusSigHandler<T> handler) throws DBusException;

	/**
	 * Add a Signal Handler. Adds a signal handler to call when a signal is
	 * received which matches the specified type and name.
	 * 
	 * @param type
	 *            The signal to watch for.
	 * @param handler
			...
			DBusSigHandler<T> handler) throws DBusException
	{
		if (!DBusSignal.class.isAssignableFrom(type))
			throw new ClassCastException(_("Not A DBus Signal"));
		addSigHandler(new DBusMatchRule(type),
				(DBusSigHandler<? extends DBusSignal>) handler);
	}

	/**
	 * Add a Signal Handler. Adds a signal handler to call when a signal is
	 * received which matches the specified type, name and object.
	 * 
	 * @param type
	 *            The signal to watch for.
	 * @param object
			...
			throws DBusException
	{
		...
		if (!objectpath.matches(OBJECT_REGEX)
				|| objectpath.length() > MAX_NAME_LENGTH)
			throw new DBusException(_("Invalid object path: ") + objectpath);
		addSigHandler(new DBusMatchRule(type, null, objectpath),
				(DBusSigHandler<? extends DBusSignal>) handler);
	}

	protected abstract <T extends DBusSignal> void addSigHandler(
			DBusMatchRule rule, DBusSigHandler<T> handler) throws DBusException;

	protected <T extends DBusSignal> void addSigHandlerWithoutMatch(
			Class<? extends DBusSignal> signal, DBusSigHandler<T> handler)
			throws DBusException
	{
	...
	public void disconnect()
	{
		connected = false;
		if (Debug.debug)
			Debug.print(Debug.INFO, "Sending disconnected signal");
		try {
			handleMessage(new org.freedesktop.DBus.Local.Disconnected("/"));
		} catch (Exception ee) {
			if (EXCEPTION_DEBUG && Debug.debug)
				Debug.print(Debug.ERR, ee);
		}

		if (Debug.debug)
			Debug.print(Debug.INFO, "Disconnecting Abstract Connection");
	...
	/**
	 * Call a method asynchronously and set a callback. This handler will be
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <A> void callWithCallback(DBusInterface object, String m,
			CallbackHandler<A> callback, Object... parameters)
	{
		if (Debug.debug)
			Debug.print(Debug.VERBOSE,
					"callWithCallback(" + object + "," + m + ", " + callback);
		...
		try {
			...
			RemoteInvocationHandler.executeRemoteMethod(ro, me, this,
					RemoteInvocationHandler.CALL_TYPE_CALLBACK, callback,
					parameters);
	...
	/**
	 * Call a method asynchronously and get a handle with which to get the
	 * reply.
	 * 
	 * @param object
	 *            The remote object on which to call the method.
	 ...
	 */
	@SuppressWarnings("unchecked")
	public DBusAsyncReply callMethodAsync(DBusInterface object, String m,
			Object... parameters)
	{
		Class<?>[] types = new Class[parameters.length];
		for (int i = 0; i < parameters.length; i++)
		...
		try {
			...
			return (DBusAsyncReply) RemoteInvocationHandler.executeRemoteMethod(
					ro, me, this, RemoteInvocationHandler.CALL_TYPE_ASYNC, null,
					parameters);
		} catch (DBusExecutionException DBEe) {
			if (EXCEPTION_DEBUG && Debug.debug)
				Debug.print(Debug.ERR, DBEe);
	...
	{
		if (Debug.debug)
			Debug.print(Debug.DEBUG, "Handling incoming method call: " + m);
		...
		if (null == m.getInterface()
				...
				|| m.getInterface()
						.equals("org.freedesktop.DBus.Introspectable")) {
			synchronized (exportedObjects) {
		...
		if (null == o) {
			...
			if (null != eo && null == eo.object.get()) {
				if (Debug.debug)
					Debug.print(Debug.INFO,
							"Unexporting " + m.getPath() + " implicitly");
			...
			if (null == eo) {
				try {
					queueOutgoing(new Error(m, new DBus.Error.UnknownObject(m
							.getPath()
							+ _(" is not an object provided by this process."))));
				} catch (DBusException DBe) {
				}
				return;
			}
			if (Debug.debug) {
				Debug.print(Debug.VERBOSE, "Searching for method " + m.getName()
						+ " with signature " + m.getSig());
				Debug.print(Debug.VERBOSE, "List of methods on " + eo + ":");
				for (MethodTuple mt : eo.methods.keySet())
					Debug.print(Debug.VERBOSE,
							"   " + mt + " => " + eo.methods.get(mt));
			}
			meth = eo.methods.get(new MethodTuple(m.getName(), m.getSig()));
			if (null == meth) {
				try {
					queueOutgoing(new Error(m,
							new DBus.Error.UnknownMethod(MessageFormat.format(
									_("The method `{0}.{1}' does not exist on this object."),
									new Object[] { m.getInterface(),
											m.getName() }))));
				} catch (DBusException DBe) {
				}
				return;
		...
		addRunnable(new Runnable() {
			...
			public synchronized void run()
			{
				...
				if (Debug.debug)
					Debug.print(Debug.DEBUG,
							"Running method " + me + " for remote call");
				try {
					Type[] ts = me.getGenericParameterTypes();
					m.setArgs(Marshalling.deSerializeParameters(
							m.getParameters(), ts, conn));
					if (Debug.debug)
						Debug.print(Debug.VERBOSE, "Deserialised "
								+ Arrays.deepToString(m.getParameters())
								+ " to types " + Arrays.deepToString(ts));
				} catch (Exception e) {
					if (EXCEPTION_DEBUG && Debug.debug)
						Debug.print(Debug.ERR, e);
					try {
						conn.queueOutgoing(new Error(m,
								new DBus.Error.UnknownMethod(
										_("Failure in de-serializing message: ")
												+ e)));
					} catch (DBusException DBe) {
					}
					return;
				...
				try {
					...
					try {
						if (Debug.debug)
							Debug.print(Debug.VERBOSE, "Invoking Method: " + me
									+ " on " + ob + " with parameters "
									+ Arrays.deepToString(m.getParameters()));
					...
					if (!noreply) {
						...
						if (Void.TYPE.equals(me.getReturnType()))
							...
							for (String s : Marshalling
									.getDBusType(me.getGenericReturnType()))
								sb.append(s);
							Object[] nr = Marshalling.convertParameters(
									new Object[] { result },
					...
					try {
						conn.queueOutgoing(new Error(m,
								new DBusExecutionException(MessageFormat.format(
										_("Error Executing Method {0}.{1}: {2}"),
										new Object[] { m.getInterface(),
												m.getName(), e
														.getMessage() }))));
					} catch (DBusException DBe) {
	...
	{
		if (Debug.debug)
			Debug.print(Debug.DEBUG, "Handling incoming signal: " + s);
		...
		synchronized (handledSignals) {
			...
			if (null != t)
				v.addAll(t);
			t = handledSignals.get(new SignalTuple(s.getInterface(),
					s.getName(), s.getPath(), null));
			if (null != t)
				v.addAll(t);
			t = handledSignals.get(new SignalTuple(s.getInterface(),
					s.getName(), null, s.getSource()));
			if (null != t)
				v.addAll(t);
			t = handledSignals.get(new SignalTuple(s.getInterface(),
					s.getName(), s.getPath(), s.getSource()));
			if (null != t)
				v.addAll(t);
		}
		if (0 == v.size())
		...
		for (final DBusSigHandler<? extends DBusSignal> h : v) {
			if (Debug.debug)
				Debug.print(Debug.VERBOSE, "Adding Runnable for signal " + s
						+ " with handler " + h);
			addRunnable(new Runnable() {
				private boolean run = false;
				...
				public synchronized void run()
				{
					...
					try {
						DBusSignal rs;
						if (s instanceof DBusSignal.internalsig
								|| s.getClass().equals(DBusSignal.class))
						...
						try {
							conn.queueOutgoing(new Error(s,
	...
	{
		if (Debug.debug)
			Debug.print(Debug.DEBUG, "Handling incoming error: " + err);
		MethodCall m = null;
		...
		if (null != m) {
			...
			if (null != cbh) {
				...
				if (Debug.debug)
					Debug.print(Debug.VERBOSE,
							"Adding Error Runnable with callback handler "
									+ fcbh);
				addRunnable(new Runnable() {
					private boolean run = false;
					...
					public synchronized void run()
					{
						...
						try {
							if (Debug.debug)
								Debug.print(Debug.VERBOSE,
										"Running Error Callback for " + err);
	...
	{
		if (Debug.debug)
			Debug.print(Debug.DEBUG, "Handling incoming method return: " + mr);
		MethodCall m = null;
		if (null == pendingCalls)
			return;
		synchronized (pendingCalls) {
		...
		if (null != m) {
			...
			if (null != cbh) {
				...
				if (Debug.debug)
					Debug.print(Debug.VERBOSE,
							"Adding Runnable for method " + fasr.getMethod()
									+ " with callback handler " + fcbh);
				addRunnable(new Runnable() {
					private boolean run = false;
					...
					public synchronized void run()
					{
						...
						try {
							if (Debug.debug)
								Debug.print(Debug.VERBOSE,
										"Running Callback for " + mr);
							DBusCallInfo info = new DBusCallInfo(mr);
							synchronized (infomap) {
								infomap.put(Thread.currentThread(), info);
							}

							fcbh.handle(RemoteInvocationHandler.convertRV(
									mr.getSig(), mr.getParameters(),
			...
			try {
				queueOutgoing(new Error(mr, new DBusExecutionException(_(
		...
		try {
			...
			if (m instanceof MethodCall && e instanceof DBusExecutionException)
				...
				try {
					if (Debug.debug)
						Debug.print(Debug.INFO,
								"Setting reply to " + m + " as an error");
