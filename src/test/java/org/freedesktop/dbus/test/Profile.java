	...
	public static void main(String[] args)
	{
		try {
			...
			if ("pings".equals(args[0])) {
				...
				System.out.println("min/max/avg (ms): " + l.min() + "/"
						+ l.max() + "/" + l.mean());
				...
				System.out.print("Sending array of " + STRING_ARRAY_LENGTH
						+ " strings " + count + " times.");
				...
				System.out.print("Sending array of " + ARRAY_LENGTH + " ints "
						+ count + " times.");
				ProfilerInstance pi = new ProfilerInstance();
				conn.exportObject("/Profiler", pi);
				Profiler p = conn.getRemoteObject(
				...
				System.out.print("Sending map of " + MAP_LENGTH
						+ " string=>strings " + count + " times.");
				ProfilerInstance pi = new ProfilerInstance();
				conn.exportObject("/Profiler", pi);
				Profiler p = conn.getRemoteObject(
						"org.freedesktop.DBus.java.profiler", "/Profiler",
				...
				System.out.print("Sending list of " + LIST_LENGTH + " strings "
						+ count + " times.");
				ProfilerInstance pi = new ProfilerInstance();
				conn.exportObject("/Profiler", pi);
				Profiler p = conn.getRemoteObject(
				...
				Profiler p = conn.getRemoteObject(
						"org.freedesktop.DBus.java.profiler", "/Profiler",
						Profiler.class);
				ProfileStruct ps = new ProfileStruct("hello", new UInt32(18),
						500L);
				Log l = new Log(count);
				long t = System.currentTimeMillis();
				...
				System.out.print(
						"Recieving introspection data " + count + " times.");
				ProfilerInstance pi = new ProfilerInstance();
				conn.exportObject("/Profiler", pi);
				Introspectable is = conn.getRemoteObject(
						"org.freedesktop.DBus.java.profiler", "/Profiler",
				...
				p.bytes(bs);
				System.out.println(
						" done in " + (System.currentTimeMillis() - t) + "ms.");
				...
				Profiler p = conn.getRemoteObject(
						"org.freedesktop.DBus.java.profiler", "/Profiler",
						Profiler.class);
				Peer peer = conn.getRemoteObject(
						"org.freedesktop.DBus.java.profiler", "/Profiler",
						Peer.class);
				...
				System.out.println("No payload: "
						+ ((count * 1000) / (end - start)) + " RT/second");
				...
				System.out.println("No payload, One way: "
						+ ((count * 1000) / (end - start)) + " /second");
				int len = 256;
				while (len <= 32768) {
					...
					System.out.println(
							len + " byte array) " + (count * len) + " bytes in "
									+ ms + "ms (in " + count + " calls / "
									+ (int) cps + " CPS): " + rate + "MB/s");
					len <<= 1;
				}
				len = 256;
				while (len <= 32768) {
				...
				System.out.println(
						"Syntax: profile <pings|arrays|introspect|maps|bytes|lists|structs|signals>");
				System.exit(1);
			}
			conn.disconnect();
		} catch (Exception e) {
