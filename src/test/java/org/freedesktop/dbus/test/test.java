		...
		TestSignalInterface, TestSignalInterface2, Properties
{
			...
			TestSerializable<String> s, int i)
	{
		...
		if (b != 12 || i != 13 || !(s.getInt() == 1)
				...
				|| !(s.getVector().size() == 3) || !(s.getVector().get(0) == 1)
				|| !(s.getVector().get(1) == 2) || !(s.getVector().get(2) == 3))
	...
	public void sig(Type[] s)
	{
		if (s.length != 2 || !s[0].equals(Byte.class)
				...
				|| !Integer.class.equals(
						((ParameterizedType) s[1]).getActualTypeArguments()[1]))
	...
	public static void main(String[] args)
	{
		try {
			...
			TestStruct3 ts3 = new TestStruct3(new TestStruct2(
					new Vector<String>(), new Variant<Integer>(0)), lli);
			...
			DBusAsyncReply<Boolean> stuffreply = (DBusAsyncReply<Boolean>) clientconn
					.callMethodAsync(tri2, "dostuff",
							new TestStruct("bar", new UInt32(52),
